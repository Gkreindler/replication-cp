...
gmm_options = Dict(
	"run_main" 					=> true, # main GMM options
		...
		"main_maxIter" 				=> 1000,
		"main_time_limit" 			=> 2 .* 3600.0,  # stop after this time
	...
	"run_boot" 					=> true,	# boot options
		...
		"boot_maxIter" 				=> 1000,
		"boot_time_limit" 			=> 2 .* 3600.0,  # stop after this time
		"boot_debug" 				=> false,
...
# activate Julia environment modules, other .jl files, define paths functions
# also defines rootpath
include("basic-imports.jl") 

Random.seed!(123)

## 1. Model 2: full model with delta = 90
	gmm_options["results_suffix"] = "_delta_90"  # leave empty by default
