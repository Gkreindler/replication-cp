...
## Paths
	...
	rootpath_output = rootpath * "model/simulation_results/power/"
...
# rt_power_range = 0.5:0.1:1.5
rt_power_range = [0.5, 1.5]

rootpath_output_folders = []
for rt_power in rt_power_range
	rootpath_output_subfolder = rootpath_output * "power_" * string(Int64(rt_power * 10)) * "/"
	push!(rootpath_output_folders, rootpath_output_subfolder)
	isdir(rootpath_output_subfolder) || mkdir(rootpath_output_subfolder)
end

for idx=1:length(rt_power_range)

	println("\n\n\n ====================================")
	println("POWER IDX ", idx)
	println("====================================\n\n\n")
	...
	run_sim(rootpath_input=rootpath_input,
			...
			rt_type="pow2",
			rt_power=rt_power_range[idx],				   ### IDX
			proptowage=false,
